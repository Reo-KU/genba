...
export const ja: Translations = {
  ...
  agentForm: {
    ...
    obsidianNotesSubdirHint: "Seitonがvault内で作成・更新するフォルダ名です。通常は MAO のままでOK。vault直下に organization / agents / tasks を置きたい場合は . を指定してください。このフォルダは先に作らなくても、組織保存やタスク開始時に自動作成されます。",
  ...
  inspector: {
    ...
    permissionPolicy: "Permission Policy",
    edit: "変更 (Edit)",
    askExecClaudeHint: "claude の権限要求は Seiton の承認ダイアログで都度確認します",
  ...
  setup: {
    ...
    installGuide: "Install ボタンで Seiton が裏側で実行します。手動でやりたい場合は Copy してターミナルへ。",
    maoWorkspaceHint: "※ タスク実行時に各 agent の workingDirectory 内に .mao/ が作られます (cleanup されます)。git 管理下なら .gitignore に自動追記。",
    missingRequired: (n: number) => `${n} 個の必須ツールが見つかりません。Seiton が正常に動作しない可能性があります。`,
